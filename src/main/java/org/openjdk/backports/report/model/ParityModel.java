...
public class ParityModel extends AbstractModel {
    ...
    public ParityModel(Clients clients, PrintStream debugOut, int majorVer) {
        ...
        for (Issue p : mp.keySet()) {
            ...
            if (firstOracle == null && firstOpen != null) {
                Map<Issue, SingleVersMetadata> map = onlyOpen.computeIfAbsent(firstOpen, k -> new TreeMap<>(DEFAULT_ISSUE_SORT));
                map.put(p, new SingleVersMetadata(clients.getRawRest(), p, majorVer, null, firstOpenRaw));
            }

            if (firstOracle != null && firstOpen == null) {
                Map<Issue, SingleVersMetadata> map = onlyOracle.computeIfAbsent(firstOracle, k -> new TreeMap<>(DEFAULT_ISSUE_SORT));
                map.put(p, new SingleVersMetadata(clients.getRawRest(), p, majorVer, firstOracleRaw, null));
            }
    ...
    public static class SingleVersMetadata {
        private final String ver;
        private final String firstOpenRaw;
        private final String firstOracleRaw;
        private final String interestTags;
        private final boolean backportRequested;
        private final Collection<String> reviewLinks;

        public SingleVersMetadata(RawRestClient rawRest, Issue issue, int majorVer, String firstOracleRaw, String firstOpenRaw) {
            ver = firstOracleRaw != null ? firstOracleRaw : firstOpenRaw;
            this.firstOpenRaw = firstOpenRaw;
            this.firstOracleRaw = firstOracleRaw;
            interestTags = InterestTags.shortTags(issue.getLabels());
            backportRequested = issue.getLabels().contains("jdk" + majorVer + "u-fix-request");
            reviewLinks = Accessors.getReviewURLs(rawRest, issue, majorVer);
        }

        public String version() {
            return ver;
        }
        public String firstOpenRaw() {
            return firstOpenRaw;
        }
        public String firstOracleRaw() {
            return firstOracleRaw;
        }
        ...
        public Collection<String> reviewLinks() {
            /* Lazy retrieval might be more efficient here. */
            return reviewLinks;
        }
    }
